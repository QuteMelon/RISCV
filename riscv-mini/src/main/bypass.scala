...
class Datapath(implicit val p: Parameters) extends Module with CoreParams {//数据通路
  ...
  val if_npc  = Mux(stall, if_pc, 
                ...
                Mux(io.ctrl.pc_sel === PC_0, if_pc,  if_pc + 4.U)))))
  ...
  val rs1 = Mux(exe_wb_sel  === WB_ALU && em_rs1hazard, alu.io.out, 
            Mux(mem_wb_sel === WB_ALU && mw_rs1hazard, mem_alu, regFile.io.rdata1))
  val rs2 = Mux(exe_wb_sel  === WB_ALU && em_rs2hazard, alu.io.out,
            Mux(mem_wb_sel === WB_ALU && mw_rs2hazard, mem_alu, regFile.io.rdata2))

  /*
  val rs1 = Mux(mem_wb_sel === WB_ALU && mw_rs1hazard, regWrite,
            Mux(exe_wb_sel  === WB_ALU && em_rs1hazard, alu.io.out,  regFile.io.rdata1))
  val rs2 = Mux(mem_wb_sel === WB_ALU && mw_rs2hazard, regWrite,
            Mux(exe_wb_sel  === WB_ALU && em_rs2hazard, alu.io.out, regFile.io.rdata2))
            */
  ...
  val mem_rd_addr    = mem_inst(11,7)
  val wb_rd_addr     = wb_inst(11,7)
  val em_rs1hazard  = mem_wb_en && exe_rs1_addr.orR && (exe_rs1_addr === mem_rd_addr)
  val em_rs2hazard  = mem_wb_en && exe_rs2_addr.orR && (exe_rs2_addr === mem_rd_addr)
  
  val mw_rs1hazard  = wb_en && exe_rs1_addr.orR && (exe_rs1_addr === wb_rd_addr)
  val mw_rs2hazard  = wb_en && exe_rs2_addr.orR && (exe_rs2_addr === wb_rd_addr)

  val rs1 = Mux(wb_wb_sel === WB_ALU && mw_rs1hazard, wb_data, 
            Mux(mem_wb_sel  === WB_ALU && em_rs1hazard, wb_alu, exe_rdata1))
  val rs2 = Mux(wb_wb_sel === WB_ALU && mw_rs2hazard, wb_data, 
            Mux(mem_wb_sel  === WB_ALU && em_rs2hazard, wb_alu, exe_rdata2))
            */
  ...
  regWrite = MuxLookup(wb_wb_sel, wb_alu.zext, Seq(
    WB_MEM -> load,
